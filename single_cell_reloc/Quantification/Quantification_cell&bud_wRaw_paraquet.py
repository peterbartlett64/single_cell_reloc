...
imgIndex = pd.read_csv('imgIndex.csv') # Inex of all images to be loaded in
...
# Info_all = pd.read_csv('info_simple.csv')
info_index = pd.read_csv("info_index.csv", error_bad_lines=False, warn_bad_lines= True).set_index("Pos")

# Info_all = pd.read_csv('info_simple.csv', error_bad_lines=False, warn_bad_lines= True)
...
for root, dirs, files, in os.walk(os.getcwd()):
	for name in files:
		if name.endswith(".csv") and name.startswith("Quantification_d"): # fix naming
...
# 	for name in files:
# 		print(name)
# 		if name.endswith(".csv") and name.startswith("Quantification"): # fix naming
# 			Quantification_index.append({'Path': os.path.join(root, name)})
# 			count = count + 1
# 			print(count, end="\r")
# 		else:
