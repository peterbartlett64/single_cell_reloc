...
def stage_from_frame(cell, frame) ->int:
...
def info_from_frame(cell, frame, info, div) ->int: #* This is a modification of the cell_stage function to capture other info
	sub = cell_cycle_df.loc[cell,:][[info]] #* Subset based on cell position_barcode and grab the relavant columns
	for i in range(len(sub)):
		#* Get required stage times to find where the current frame fits in
		g = sub.iloc[i:(i+2), :]
		g2_st1 = g.iloc[0]["g2_st"]
		...
		try:
			...
			#< Start of the conditions within the loop. 1 ==> G1/S, 2 ==> G2/M
			if frame < g2_st1:
				return(1)
			elif g2_st1 <= frame <= g2_e1:
			# distance = frame - g2_st
				return(2)
			elif g2_e1 <= frame <= g2_st2:
			# distance = frame - g2_e
				return(1)
			elif g2_st2 <= frame <= g2_e2:
			# distance = frame - g2_st
				return(2)
			else:
				continue
...
def cell_stage_assocation(position_barcode):
	try:
		cell_cycle_df = pd.read_csv(f"{position_barcode}/", usecols=["track", "parent", "daughter", "tree", "generation", "age", "g2_st", "g2_e", "g2_dur", "div", "seg_st_track", "seg_st_daughter"]) #* Removed columns which do not have information based on input to TracX
		...
		localization_df = pd.read_csv(f"Movement_treat_course_{position_barcode}.csv", usecols= ["Cell_Barcode", "Relocalized", "Frame_x"]) #! Decide if this is the correct input
...
def cell_stage_manager():#JobCount) -> None:
...
def stats_cell():
	...
	localization_df.astype({"Cell_stage": 'category', "Relocalized": "category"})
	temp_for_stats = localization_df.dropna(subset=['Cell_stage']) #* Remove cells for which we do not know the cell stage

	F, p = stats.f_oneway(temp_for_stats["Relocalized"], temp_for_stats["Cell_stage"]) #* Perform a one way ANOVA comparison of two categorical variables
	return(F,p)
