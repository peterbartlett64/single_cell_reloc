...
def reloc_yet_all(full_data):#, post_path, output_file):
	...
	working = working.sort_values(by = "Unique_Frame")
...
#%%
def load_location_info(info_path = "C:\\Users\\pcnba\\Grant Brown's Lab Dropbox\\Peter Bartlett\\Peter Bartlett Data\\Code\\Data_copies\\Localization_merging\\Tcak_protein_localization.xls"):
...
# 	Agg_loc = post_treatment_df.groupby(["Protein", "Frame"])["Loc_score_log"].agg('median') #* First get the median of Loc_scores in each protein frame
...
# 	max_agg_Loc_score = Agg_loc.loc[Agg_loc.groupby(["Protein"])["Loc_score_log"].idxmax()][["Protein", "Frame"]] #*Create a table of maximum aggregated Loc_scores for each protein
...
#%%
#Todo: Move this to a call in final merge
...
Loc_data = pd.read_parquet('D:\ALL_FINAL\Combined_by_perc\Loc_data_comp_merged.parquet')
...
cols = ['Gene_Standard_Name', 'Combined_destination', 'Combined_origin',
       ...
       'Quad_destination', 'Cell_Barcode', 'Date', 'Unique_Frame',
       'Protein', 'Unique_pos', 'ImageID', 'When']
Loc_data[cols] = Loc_data[cols].astype('category')
table = pa.Table.from_pandas(Loc_data)
pq.write_table(table, 'D:\ALL_FINAL\Combined_by_perc\Loc_data_comp_merged2.parquet')
...
def graph_pearson_single(i, full_data, max_agg_Loc_score):
	...
	try:
		sub_data = full_data.loc[(full_data["Protein"] == prot) & (full_data["Frame"] == f)].dropna(subset=['z_score_logLoc', 'z_score_logAbund'])
	except ValueError: #* I don't know how these values wouldn't exist
		sub_data["z_score_logAbund"] = stats.zscore(sub_data.loc[:,"log_Abundance"])
		sub_data["z_score_logLoc"] = stats.zscore(sub_data.loc[:,"log_Loc_score"])
	z_Loc_Abun_r = stats.pearsonr(sub_data["z_score_logLoc"], sub_data["z_score_log_Abund"])
	scatter = sns.scatterplot(data= sub_data, x = "z_score_log_Abund", y = "z_score_logLoc")#. This an the above lines were changed 11/10/2023
	fig = scatter.get_figure()
	fig.savefig(f"Pearson{z_Loc_Abun_r}--{prot}_{f}.pdf")
	print(f"{prot} is done pearson'n")
	return(prot, z_Loc_Abun_r)

def graph_pearson_all_t(i, full_data, max_agg_Loc_score):
	prot = max_agg_Loc_score.iloc[i,:]["Protein"]
	f = max_agg_Loc_score.iloc[i,:]["Frame"]
	try:
		sub_data = full_data.loc[(full_data["Protein"] == prot) & (full_data["Frame"] == f)].dropna(subset=['z_score_logLoc', 'z_score_logAbund'])
		...
		sub_data["z_score_logLoc"] = stats.zscore(sub_data.loc[:,"log_Loc_score"])
	z_Loc_Abun_r = stats.pearsonr(sub_data["z_score_logLoc"], sub_data["z_score_log_Abund"])
	scatter = sns.scatterplot(data= sub_data, x = "z_score_Abund", y = "z_score_logLoc")
	fig = scatter.get_figure()
...
def graph_pearson_plus_minus(i, full_data, max_agg_Loc_score):
	try:
		...
		f = max_agg_Loc_score.iloc[i,:]["Frame"]
		try:
			sub_data = full_data.loc[(full_data["Protein"] == prot) & ((full_data["Frame"] == f)| (full_data["Frame"] == f+1)| (full_data["Frame"] == f-1))].dropna(subset=['z_score_logLoc', 'z_score_logAbund'])
		except ValueError:
			# sub_data["z_score_Abund"] = stats.zscore(sub_data.loc[:,"Abundance"])
			# sub_data["z_score_Loc"] = stats.zscore(sub_data.loc[:,"Loc_score"])
			sub_data["z_score_logAbund"] = sub_data.groupby("Frame")["log_Abundance"].transform(stats.zscore)
			sub_data["z_score_logLoc"] = sub_data.groupby("Frame")["log_Loc_score"].transform(stats.zscore)
		z_Loc_Abun_r = stats.pearsonr(sub_data["z_score_Loc"], sub_data["z_score_Abund"])
		scatter = sns.scatterplot(data= sub_data, x = "z_score_Abund", y = "z_score_logLoc", hue = "Frame")
		fig = scatter.get_figure()
		fig.savefig(f"Pearson{z_Loc_Abun_r}--{prot}_{f}.pdf")
		print(f"{prot} is done pearson'n")
		return(prot, z_Loc_Abun_r)
...
if __name__ == "__main__":
	...
	#* Set final file name
	output_file = 'Final_wAbund.parquet'
	...
	try:
		full_data = pd.read_parquet(output_file, columns = ['Cell_Barcode','Date', 'Frame', 'Unique_Frame', 'Protein', 'Is_treated', 'Frames_post_treatment', 'Unique_pos', 'Loc_score', 'Relocalized', '<lambda_0>_x','<lambda_0>_y', 'Abundance', 'log_Abundance', 'log_Loc_score', 'z_score_Loc', 'z_score_Abund', 'z_score_logLoc', 'z_score_logAbund', 'ImageID', 'Progen_bud', 'Percentile_calc']) #* This speeds it up a TON by not loding extra information. Added column in for confirming that the correct perctile was used in calculations. This information was already stored in the file with selected_series, and uses the series with the highest penetrance,but added this for when a protein shows up multiple times like DDC2.
		...
		for filename in os.listdir(post_path):
			...
			if filename.startswith('merged'):
				...
				continue
			elif filename.__contains__('info'):
				continue
			elif filename.__contains__('percs'):
				continue
			elif filename.endswith('.parquet'):
				# Load the Parquet file into a DataFrame
				file_path = os.path.join(post_path, filename)
				df = pd.read_parquet(file_path, columns = ['Cell_Barcode','Date', 'Frame', 'Unique_Frame', 'Protein', 'Is_treated', 'Frames_post_treatment', 'Unique_pos', 'Loc_score', 'Relocalized', '<lambda_0>_x','<lambda_0>_y', 'Abundance', 'log_Abundance', 'log_Loc_score', 'z_score_Loc', 'z_score_Abund', 'z_score_logLoc', 'z_score_logAbund', 'ImageID', 'Progen_bud']).reset_index(drop = False)
		...
		merged_data.to_parquet('merged.parquet')


		# Write the merged data to a single Parquet file
		full_data = merged_data
		full_data = reloc_yet_all(full_data = full_data)
	...
	for p in after_copy['Protein'].unique():
		...
		del fig
#%%
os.chdir(figures) #* This is the updated path with the combined dir added on
def line_plot_hedge_test(prot):
	"""
	Create two different plots of cell movement to determine if there is evidence of true priming. This funciton works best when it is run in a loop for each protein

	Input(s):
		prot [str]: Protein for which to create the graphs

	Outputs:
		PDF figure of inddivual cells as: individual_lineplot_{prot}.pdf
		PDF figure of grouped prime state cells as: grouped_lineplot_{prot}.pdf
	"""
	subset_tempProt = after_copy.loc[(after_copy['Protein'] == prot) & ((after_copy['Frames_post_treatment'].isin([0,8,16,24,32,40])) | (after_copy['Frames_post_treatment']<0))]
	barcs_keep = subset_tempProt.groupby(['Unique_pos'])['Cell_Barcode'].sample(frac = 0.3)
	if len(barcs_keep) < 30:
		barcs_keep = subset_tempProt.groupby(['Unique_pos'])['Cell_Barcode'].sample(n = 30)
	yet_yes = pd.DataFrame(subset_tempProt.loc[(subset_tempProt['Yet'] == 1) & (subset_tempProt['Frames_post_treatment'] == 0)]['Cell_Barcode'])
	yes_keep = [] #yet_yes.sample(frac=0.5)['Cell_Barcode'] #! This is tempoary change to make so that all cells are used in the grouped lineplot
	if len(yes_keep) < 10:
		yes_keep = yet_yes['Cell_Barcode']
	barcs_keep = pd.concat([barcs_keep, yes_keep])

	subset_tempProt_smaller = subset_tempProt.loc[subset_tempProt["Cell_Barcode"].isin((barcs_keep))]
	plot_prot = sns.lineplot(data = subset_tempProt_smaller, x = "Time_post_treatment", y= "Loc_score")
	plot_prot.axhline(y = 1)
	plot_prot.set(xlabel= 'Time post treatment (minutes)', ylabel = 'LocScore')
	fig = plot_prot.get_figure()
	fig.savefig(f"individual_lineplot_{prot}.pdf", format = 'pdf')

	different_subset = after_copy.loc[(after_copy['Protein'] == prot)] #*This is not subsetted by time because the result will be a line. There are no groups so this will generate much smoother lines
	...
	line_prot = sns.lineplot(data = different_subset, x = 'Time_post_treatment', y = 'Loc_score', hue = 'Group')
	line_prot.axhline(y = 1)
	line_prot.set(xlabel= 'Time post treatment (minutes)', ylabel = 'LocScore')
	fig = line_prot.get_figure()
	len_check = pd.DataFrame(primed.apply(lambda x: x[:x.find("c")]))
	len_check['t'] = True #* This is just for pandas so it has a column to count elements
	len_check.groupby('Cell_Barcode')['t'].count()
	fig.savefig(f"grouped_lineplot_{prot}.pdf", format = 'pdf')
