...
interest_prots <- list('SAE2', 'RAD51', 'RMI1')
...
test <-  df_for_facet_abundance %>% 
  group_by(Protein, Frames_post_treatment) %>% 
  arrange(desc(z_score_Loc),.by_group = TRUE) %>%
  mutate(z_rank_Abund = rank(z_score_logAbund, ties.method = 'first'),
         z_rank_Loc = rank(z_score_Loc, ties.method = 'first'))
