...
def f_GO_map(test): #! This was left out because decided to display based on a network rather than colored scatter
	...
	#. This below code has not been tested
	...
	# test['GO_group_collected'] = ''
	# for r in terms:
	# 	go_matches = go.loc[r, 'ANNOTATED_GENES']
	# 	test['GO_group_collected'] = test.apply(lambda x: add_GOs(x['Protein'], x['GO_group_collected'], r, go_matches), axis = 1)
	return(go) #. , test)
