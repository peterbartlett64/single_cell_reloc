...
def combine_pos(pos):
	...
	subset = Quantification_index.loc[Quantification_index["PositionID"] == pos] #Todo:This should be set a local variable not global
...
def MI_move(p, percentile, perc_path): #* The Quant all INdex
	try: #TODO: Rename Frame_x to just Frame
		...
		Quant_prim['Percentile_calc'] = percentile #* This was added JAN 10, 2023 for future ease of function when there are several percentiles being used and for confirmation
		...
		UNT_GFP_mKa_factor_upper = np.median(UNT_mKa_GFP) + 1.5*stats.median_abs_deviation(UNT_mKa_GFP) #! THIS IS 1.5. ALL ANALYSES WERE 2
		UNT_GFP_mKa_factor_lower = np.median(UNT_mKa_GFP) - 1.5*stats.median_abs_deviation(UNT_mKa_GFP)
		...
		UNT_GFP_mKO_factor_upper = np.median(UNT_mKO_GFP) + 1.5*stats.median_abs_deviation(UNT_mKO_GFP)
		UNT_GFP_mKO_factor_lower = np.median(UNT_mKO_GFP) - 1.5*stats.median_abs_deviation(UNT_mKO_GFP)
		...
		Raw_factorUpper_mKa = np.median(UNT_mKa[fluor_perc_r]) + 1.5*stats.median_abs_deviation(UNT_mKa[fluor_perc]) #This calculated the parameters based on the variable percentile. By writing in this way, I can test different percentile for differnt loc types
		Raw_factorLower_mKa = np.median(UNT_mKa[fluor_perc_r]) - 1.5*stats.median_abs_deviation(UNT_mKa[fluor_perc])
		Raw_factorUpper_mKO = np.median(UNT_mKO[fluor_perc_r]) + 1.5*stats.median_abs_deviation(UNT_mKO[fluor_perc])
		Raw_factorLower_mKO = np.median(UNT_mKO[fluor_perc_r]) - 1.5*stats.median_abs_deviation(UNT_mKO[fluor_perc])
...
def new_percentages_post_t(chamber, Chamber_df, date_found): # This function calculated the percentages with just the post-treatment values
	...
	try:
		...
		#< Added NOV23
		Chamber_df_local_rem["Yet"] = np.nan
		Chamber_df_local_rem["Does"] = np.nan
		Chamber_df_local_rem["When"] = None
		...
		Chamber_df_local_rem["Does"] = Chamber_df_local['Does'] = Chamber_df_local.groupby(by = "Cell_Barcode")["Does"].bfill()
		Chamber_df_local['When'] = Chamber_df_local.groupby(by = "Cell_Barcode")["When"].bfill()
...
if __name__ == '__main__':
	...
	pr = Global_variables["cpu_se"] #* This can be changed
	...
	Condition_information = pd.read_excel("C:/Users/pcnba/Grant Brown's Lab Dropbox/Peter Bartlett/Peter Bartlett Data/Code/Data_copies/Information_files/Localization_merging/MicrofluidicsMap_wCol_USE.xlsx", sheet_name='ProteinLocations', dtype={'Date' : str})[["Date", "Run Number", "Time_true", "MapID (Col_Range)", "Myo1Marker", "Protein", "Time"]]
	...
	for percentile in list_percentiles:
		...
		if len(movement_unif_index) > 20: #. Arbitrarily set but reasonable considering most machines will have 8+ cores and fewer than 20 cores. Unless you can make use of 1-2 cycles, it does not make sense to lauch parrallel flow
			...
			#Todo: change to run based on the number to complete
			Col_len = len(Col_list) #* This is refering to the column list in conidition information
		...
		date_found = f_date_find(path = str(Chamber_index.iloc[0,0]))
		...
		for c in Chamber_index["Chamber"]:
			try:
				...
				print(f"Percentage file missing for {c}")
				continue
		...
		All_pos_allt_t_percentages_melt.to_parquet("All_pos_allt_t_percentages_melt.parquet")
		...
		print(f'Done {percentile} percentile')
...
#%%
# with open("C:/Users/pcnba/Grant Brown's Lab Dropbox/Peter Bartlett/Peter Bartlett Data/Code/single_cell_reloc/single_cell_reloc_parquet/Post_quant/best_perc_SEP16.py") as f:
#     exec(f.read())

# with open("C:/Users/pcnba/Grant Brown's Lab Dropbox/Peter Bartlett/Peter Bartlett Data/Code/single_cell_reloc/single_cell_reloc_parquet/Post_quant/Intensity_select_percentage_combine.py") as f:
#     exec(f.read())
