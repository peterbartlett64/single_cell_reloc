...
def MI_move(p, percentile, perc_path): #* The Quant all INdex
	try: #TODO: Rename Frame_x to just Frame
		...
		Quant_prim["Percentile_lessBack"] = Quant_prim[f"x{percentile}thPercentile_GFP_RAW"] - Quant_prim["factor_GFP_background_Med"]
		Quant_prim["OBJFactor_lessBack"] = Quant_prim["factor_median_OBJ_GFP"] - Quant_prim["factor_GFP_background_Med"]
		Quant_prim = Quant_prim.loc[Quant_prim["OBJFactor_lessBack"] >=0]

		Quant_prim['ObjPercentileLessBackNormOBJ'] = (Quant_prim["Percentile_lessBack"]) / (Quant_prim["OBJFactor_lessBack"])
		Quant_prim.rename(columns = {"Percentile_lessBack": f"x{percentile}Percentile_lessBack", "ObjPercentileLessBackNormOBJ": f"OBJx{percentile}PercentileLessBackNormOBJ"}, inplace = True)
		...
		fluor_perc = (f"OBJx{percentile}PercentileLessBackNormOBJ")#(f"x{percentile}thPercentile_norm_OBJ_Median_GFP")
		...
		UNT_GFP_mKa_factor_upper = np.median(UNT_mKa_GFP) + 2*stats.median_abs_deviation(UNT_mKa_GFP) #! THIS IS 1.5. ALL ANALYSES WERE 2
...
if __name__ == '__main__':
	...
	'post_path': 'D:/MAD2_lessBackground', #. gv.slash_switch(input("Post quant path?")) , #Todo: This needs to be changed to a input call
	'subset': False,
	'subset_by': '',
	'subset_collection': '',
