...
#%%
def I_move(p, percentile, perc_path):
	try: #TODO: Rename Frame_x to just Frame
		...
		MMS_mKa.loc[:,"mKa_factor_upper"] = UNT_GFP_mKa_factor_upper
		MMS_mKa.loc[:,"mKa_factor_lower"] = UNT_GFP_mKa_factor_lower

		MMS_mKO = Quant_prim.loc[(Quant_prim["Myo1Identity"] == "Myo1_mKO") & (Quant_prim["Is_treated"] == 1)].copy()
		MMS_mKO.loc[:,"mKO_factor_upper"] = UNT_GFP_mKO_factor_upper
		MMS_mKO.loc[:,"mKO_factor_lower"]= UNT_GFP_mKO_factor_lower
		...
		MMS_mKO.loc[:,["Loc_score", "Upper", "Lower"]] = MMS_mKO.apply(reloc_score_mKO, axis = 1, result_type = 'expand')
...
if __name__ == '__main__':
	...
	'post_path': 'D:/Microfluidics/2023-06-19',
	'subset': False,
	'subset_by': '',
	'subset_collection': '',
	'cpu_se': 12,
	...
	for percentile in list_percetiles:
		...
		Parallel(n_jobs=pr, verbose = 100)(delayed(I_move)(p = p, percentile = percentile,perc_path = path_save) for p in range(len(Quant_prim_index)))
		#####
		...
		new_percentages_post_t()

		date_found = Chamber_index.iloc[0,0][-14:-4] #! What does this do?
