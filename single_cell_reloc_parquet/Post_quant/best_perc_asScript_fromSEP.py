...
#! These variables do not appear to be referenced?
post_quant_root = slash_switch(input("where are the files stored?")) #* This is the root file
# post_quant_sub_95 = slash_switch(input("Where are the 95th percentile files? Folder name"))
os.chdir(post_quant_root)
post_quant_sub_95 = "95th_percentile"

post_quant_sub_95 = os.path.join(post_quant_root, post_quant_sub_95)

# post_quant_sub_99 = slash_switch(input("Where are the 99th percentile files? Folder name"))
post_quant_sub_99 ="99th_percentile"
post_quant_sub_99 = os.path.join(post_quant_root, post_quant_sub_99)

measure_one = 'Percentage_reloc' #* this is the percentage of cells which are currently relocalized within a given frame
measure_two = 'Percentage_reloc_less' #* this is the percentage of cells that have relocalized thus far

first_name = 'All_pos_pt_percentages_melt.parquet'
second_name = 'All_pos_pt_t_less_percentages_melt.parquet'


#%%
# measure_one = input("What are the values being compared? (This is the merged so it should be Percenage_reloc)")
# file_One = input("File One (95)") + ".parquet"
# file_Two = input("File Two (99)") + ".parquet"
# file_One = slash_switch(input("File One (95)")) #* This is the _all file
# file_Two = slash_switch(input("File Two (99)"))

#%%
file1_ninetyfive = pd.read_parquet(slash_switch(
	os.path.join(post_quant_sub_95,first_name)))
...
file2_ninetynine = pd.read_parquet(slash_switch(os.path.join(post_quant_sub_99,first_name)))
...
#create new column with which has a larger percentage
master['larger(selected)'] = np.select(conditions, choices, default='ninetynine') #* why did I set the default as 99?
master.reset_index(drop=False, inplace = True)

def ddc_2_deal(x, current):
	if "DDC2" in x:
		return('ninetynine')
	else:
		return(current)

master['larger(selected)'] = master.apply(lambda x: ddc_2_deal(x['Protein'], x['larger(selected)']), axis = 1)
master.set_index("Protein", inplace=True)
#%%
def result_give(val1, val2, selector):
	if selector == "ninetyfive":
		return(val1, selector)
