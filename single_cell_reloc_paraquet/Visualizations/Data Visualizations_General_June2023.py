...
# # from joblib import Parallel, delayed
# # import seaborn as sns
import single_cell_reloc_paraquet.global_functions.global_variables as gv #* This version
import plotly
...
if __name__ == "__main__":
	...
	# microfluidic_results = gv.slash_switch(microfluidic_results)
	post_path = gv.slash_switch(post_path)
	os.chdir(post_path)
...
figures_root = "C:\\Users\\pcnba\\Grant Brown's Lab Dropbox\\Peter Bartlett\\Peter Bartlett Data\\Figures_new\\Direct_output"
# figures_root = gv.slash_switch(input("Where is the folder for figure outputs?"))
...
#, Read in the all the collected data in either csv or parquet form
x=0
t=0
while x == 0:
	f_type = input("csv or parquet?")
	if t == 0:
		full_data_name = "Final_wAbund"
	else:
...
#%% Read in the location information for the proteins.
location_information = pd.read_excel("C:\\Users\\pcnba\\Grant Brown's Lab Dropbox\\Peter Bartlett\\Peter Bartlett Data\\Code\\Data_copies\\Tcak_protein_localization.xls", sheet_name='Calls')
...
#%%
# os.chdir(figures)
# full_data_wLocal = pd.read_parquet("Final_wAbundLocal.parquet")

#%%
Abundance_df = full_data_wLocal.copy().reset_index(inplace = True, drop = True)
#%%
#, Abundance-Loc_pearson_r
# median_pearson = Abundance_df.groupby(["Protein"])["Pearson_coeff_Loc_Abund"].agg(median) #* Get the median level of association for all frames by protein
# max_pearson = Abundance_df.groupby(["Protein"])['Pearson_coeff_Loc_Abund'].agg('max') #* Get the maximum level of association for all frames by protein

#Todo: Finish making this or determine if there is no point
# def make_bar(agg_way, input, coloring = False, by = None): #* Get output bar based on the aggregation and coloring
# 	os.chdir(figures)

agg_way =  ['median', 'max']
os.chdir(figures)
for ag in agg_way: #* It might be a better idea to zip execute with color
	agg_pearson = Abundance_df.groupby(['Protein'])['Pearson_coeff_Loc_Abund'].agg(ag)
	pearson_coeff_bar = px.bar(agg_pearson.sort_values())
	pearson_coeff_bar.write_html(f'Pearson_r_bar_mono_{ag}.html')
	# pearson_coeff_bar.write_image(f'Pearson_r_bar_mono_{ag}.svg') #* This seems to be crashing the environment??

#%%
os.chdir(post_path)
agg_pearson = Abundance_df.groupby(['Protein'])['Pearson_coeff_Loc_Abund'].agg('median').reset_index(drop = False)
agg_pearson_wLocal = pd.merge(agg_pearson, localization_simple, left_on= 'Protein', right_index= True)
agg_pearson_wLocal.dropna(subset = 'Pearson_coeff_Loc_Abund', inplace = True)
agg_pearson_wLocal.to_parquet('temp.parquet') #* Save the temporary file in the post_path, for later review

#%%
os.chdir(figures) #* Make sure that the file is output to the direct spot
pearson_coeff_bar_location = px.bar(agg_pearson_wLocal.sort_values(by = ['Location_simplfied', 'Pearson_coeff_Loc_Abund']), x = "Protein", y = 'Pearson_coeff_Loc_Abund', color = 'Location_simplfied')
#%%
# pearson_coeff_bar_location.write_html(f'Pearson_r_bar_location.html')
pearson_coeff_bar_location.show()
pearson_coeff_bar_location.write_image('Pearson_coeff_Loc_Abund.svg')
pearson_coeff_bar_location.write_image('Pearson_coeff_Loc_Abund.eps')

#%%
# pearson_coeff_bar_location.to_image('test.', format = 'svg')

# pearson_coeff_bar_location = px.bar(median_pearson, color = 'Location_simplfied')
# pearson_sorted_absolute_bar = px.bar(median_pearson, color='absoultue_count')
# pearson_sorted_absolute_bar.write_image('Pearson_r_bar_absCount.svg')
# pearson_sorted_absolute_bar.write_image('Pearson_r_bar_absCount.eps')
