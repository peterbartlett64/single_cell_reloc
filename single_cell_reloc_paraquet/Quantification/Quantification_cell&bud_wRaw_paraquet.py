...
instances = ['d0212r1p150100', 'd0212r1p150200', 'd0212r1p150300']
instances = ['d0212r1p140300']
...
os.chdir(microfluidics_results)
# imgIndex = pd.read_hdf('MASTER.h5', 'imgIndex')
imgIndex = pd.read_paraquet('imgIndex.paraquet') # Inex of all images to be loaded in

# Allmasks = pd.read_hdf('MASTER.h5', 'Allmasks')
Allmasks = pd.read_paraquet('Allmasks.paraquet') # Index of masks which show the cell space for internal fluorescence
Allmasks_exp = pd.read_paraquet('Allmasks_exp.paraquet') #Index of masks which show the expanded cell space for budneck fluorescence
# Info_all = pd.read_hdf('MASTER.h5', 'info_simp')
# Info_all = pd.read_paraquet('info_simple.paraquet')
info_index = pd.read_paraquet("info_index.paraquet", error_bad_lines=False, warn_bad_lines= True).set_index("Pos")

# Info_all = pd.read_paraquet('info_simple.paraquet', error_bad_lines=False, warn_bad_lines= True)
...
def Quant_frame(i):
	...
	try:
		...
		def pos_info_read(unique_pos):
			...
			# too_short_drop.to_paraquet(f"Dropped_bc_short_{curdate}")
		...
		# objMeas_DF.to_paraquet(f"Quantification_{i}_{today}.paraquet")
...
for root, dirs, files, in os.walk(os.getcwd()):
	for name in files:
		if name.endswith(".paraquet") and name.startswith("Quantification_d"): # fix naming
...
# 	for name in files:
# 		print(name)
# 		if name.endswith(".paraquet") and name.startswith("Quantification"): # fix naming
# 			Quantification_index.append({'Path': os.path.join(root, name)})
# 			count = count + 1
# 			print(count, end="\r")
# 		else:
